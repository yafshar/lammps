...
void KimCreateBox::command(int narg, char **arg)
{
  ...
  if (!modify->get_fix_by_id("KIM_MODEL_STORE"))
    error->all(FLERR, "Must use 'kim init' before 'kim create_box'");

  auto tmp = input->variable->retrieve("kim_bonded_ff");
  if (!tmp) error->all(FLERR, "Use of 'kim create_box' for non-bonded SMs");
  ...
  if (ntypes < 0)
    error->all(FLERR, "number-atom-types is not provided in the KIM simulator");
  ...
  if (tmp) {
    ...
    if (nbondtypes < 0)
      error->all(FLERR, "number-bond-types is not provided in the KIM simulator");
  ...
  if (tmp) {
    ...
    if (nangletypes < 0)
      error->all(FLERR, "number-angle-types is not provided the KIM simulator");
  ...
  if (tmp) {
    ...
    if (ndihedraltypes < 0)
      error->all(FLERR, "number-dihedral-types is not provided the KIM simulator");
  ...
  if (tmp) {
    ...
    if (nimpropertypes < 0)
      error->all(FLERR, "number-improper-types is not provided the KIM simulator");
